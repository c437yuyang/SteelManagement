...
255, 255);
        ...
        public static void MakeThumbnail(string originalImagePath, string thumbnailPath, double ratio)
        {
            ...
            Bitmap bitmap = MakeThumbnail(originalImage, ratio);
            try
            {
                //以jpg格式保存缩略图
        ...
        /// <param name="ratio"></param>
        /// <returns></returns>
        public static Bitmap MakeThumbnail(Image originalImage, double ratio)
        {
            if (ratio > 1.0)
            {
                ratio = 1.0 / ratio;
            }
            return MakeThumbnail(originalImage, (int)(originalImage.Width * ratio), (int)(originalImage.Height * ratio), "WH");
        }


        public static Bitmap MakeThumbnail(Image originalImage, int thumbWidth, int thumbHeight, string mode)
        {
            ...
            switch (mode)
            {
                case "HW":  //指定高宽缩放（可能变形）                
                    break;
                case "W":   //指定宽，高按比例                    
                    toheight = originalImage.Height * thumbWidth / originalImage.Width;
                    break;
                case "H":   //指定高，宽按比例
                    towidth = originalImage.Width * thumbHeight / originalImage.Height;
                    break;
                case "Cut": //指定高宽裁减（不变形）                
                    ...
                    else
                    {
                        ...
                        oh = originalImage.Width * thumbHeight / towidth;
                        x = 0;
        ...
        public static void MakeThumbnail(string originalImagePath, string thumbnailPath, int width, int height, string mode)
        {
            ...
            Bitmap bitmap = MakeThumbnail(originalImage, width, height, mode);
            try
            {
                //以jpg格式保存缩略图
                bitmap.Save(thumbnailPath, System.Drawing.Imaging.ImageFormat.Jpeg);
        ...
        public static Bitmap KiRotate90(Bitmap img, int angle)
        {
            switch (angle)
            {
                case 90:
        ...
        public static void getImageInfo(string imgPath, out int imgWidth, out int imgHeight)
        {
